...
import { getScheduledState, scheduledStateLabel } from '@personal/services/personalTaskSchedule';
...
export function MilestoneDetailPage() {
  ...
  // Optimistic set of task IDs marked complete locally (before getRoadmap resolves)
  ...
  // Reset optimistic overrides whenever the server data refreshes
  useEffect(() => {
  ...
  const tasks = useMemo(() => {
    ...
    return activeRoadmap.tasks
      ...
      .map(t => optimisticCompleted.has(t.id) ? { ...t, status: 'completed' as const } : t)
      .sort((a, b) => {
        const o: Record<string, number> = { 'in-progress': 0, todo: 1, blocked: 2, completed: 3 };
        return (o[a.status] ?? 1) - (o[b.status] ?? 1);
      });
  }, [activeRoadmap?.tasks, milestoneId, optimisticCompleted]);

  const completedCount = tasks.filter(t => t.status === 'completed').length;
  // Use server-computed progress when available; fall back to local ratio while getRoadmap is in-flight
  const serverProgress = safeProgress(milestone?.progress);
  const localProgress = tasks.length > 0 ? Math.round((completedCount / tasks.length) * 100) : 0;
  const progress = optimisticCompleted.size > 0 ? localProgress : serverProgress;
