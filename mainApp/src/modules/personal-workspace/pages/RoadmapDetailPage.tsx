...
import { ConfirmDialog } from '@personal/components/roadmap/ConfirmDialog';
import { EditRoadmapModal } from '@personal/components/roadmap/EditRoadmapModal';
import { PhaseFormModal } from '@personal/components/roadmap/PhaseFormModal';
...
export function RoadmapDetailPage() {
  ...
  // Re-fetch when the user navigates back to this page (e.g. after completing
  // tasks in MilestoneDetailPage) so progress bars are always up-to-date.
  useEffect(() => {
    if (!id) return;
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') getRoadmap(id);
    };
  ...
  }, [roadmap?.progress, roadmap?.status, roadmap?.targetDate, roadmap?.startDate, roadmap?.createdAt]);
