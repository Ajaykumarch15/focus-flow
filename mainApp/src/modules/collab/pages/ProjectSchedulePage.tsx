...
export function ProjectSchedulePage() {
  ...
  useEffect(() => {
    ...
    if (viewMode === 'day') {
      fetchWorkspaceSchedules({ workspaceId, projectId, date: selectedDate });
    } else {
      const weekDates = getWeekDates(selectedDate);
      fetchWorkspaceSchedules({ workspaceId, projectId, from: weekDates[0], to: weekDates[6] });
    }
  }, [workspaceId, projectId, selectedDate, viewMode, fetchWorkspaceSchedules]);
  ...
  const displayMembers = useMemo(() => {
    if (selectedUserIds.length > 0) {
      return projectMembers.filter((m) => selectedUserIds.includes(m.id));
    }
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-surface-400 mt-0.5">Project schedule and task assignments</p>
            </div>
          </div>
