...
const SwarmCursor = ({
  ...
  const propsRef = useRef<Required<Omit<SwarmCursorProps, 'children' | 'className' | 'style'>>>({} as Required<Omit<SwarmCursorProps, 'children' | 'className' | 'style'>>);
  propsRef.current = {
    color,
    accentColor,
    count,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const orphan = container.querySelector('canvas');
