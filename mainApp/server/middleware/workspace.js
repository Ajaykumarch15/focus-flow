...
//   • requireAdmin (admin|superadmin) → workspace settings + membership management
//   • requireSuperadmin → workspace deletion / role mutation
...
function memberRole(ws, userId) {
  ...
  return LEGACY_ROLE_MAP[m.role] || m.role;
}
...
function requireRole(...roles) {
  return (req, res, next) => {
    if (req.member) {
      const memberRoleValue = LEGACY_ROLE_MAP[req.member.role] || req.member.role;
      if (roles.includes(memberRoleValue)) return next();
    }
...
const requireSuperadmin = requireRole(...SUPERADMIN_ROLES);
...
//   • requireWorkspaceMember    — any role (reads)
//   • requireWorkspaceEditor    — any role except nonadmin (create/update)
//   • requireWorkspaceAdmin     — admin|superadmin (delete/settings)
const requireWorkspaceMember = requireMember;
const requireWorkspaceEditor = requireRole(...ADMIN_ROLES);
const requireWorkspaceAdmin = requireRole(...ADMIN_ROLES);
...
module.exports = {
  ...
  requireWorkspaceOwnerAdmin: requireWorkspaceAdmin,
  MANAGER_ROLES: ADMIN_ROLES,
  EDITOR_ROLES: ADMIN_ROLES,
};
